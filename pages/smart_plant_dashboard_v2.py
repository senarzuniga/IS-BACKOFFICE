...
Merged: includes the interactive hotspots, KPIs and right-hand details
panel from the original dashboard and V2-specific product lookup helpers.
"""
...
def render_plant(config: dict, highlight_ids=None, height: int = 600) -> str | None:
    """Render plant image with interactive hotspots. Returns the clicked hotspot id (if any)."""
    ...
    html += """
    <script>
    // If Streamlit is not available, clicking will postMessage to the parent.
    </script>
    """
    ...
    try:
        clicked = components.html(html, height=height, scrolling=False)
    except Exception:
        try:
    ...
    with c1:
        if use_3d:
            try:
                ...
                clicked = render_plant(config, highlight_ids=highlight_ids, height=600)
        else:
            clicked = render_plant(config, highlight_ids=highlight_ids, height=600)
