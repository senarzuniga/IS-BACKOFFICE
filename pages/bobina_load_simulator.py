"""BOBINA LOAD SIMULATOR — Panel Streamlit

Panel independiente para comparar dos escenarios de carga de bobinas:
...
el motor de simulación en `core/bobina_simulation_engine.py` y los agentes
en `agents/bobina_simulator/`.
...
DATA_DIR = ROOT / "data" / "bobina_simulator"
...
from core.bobina_simulation_engine import SimulationEngine
from agents.bobina_simulator.config_agent import ConfigAgent
from agents.bobina_simulator.work_order_agent import WorkOrderAgent
from agents.bobina_simulator.roi_agent import ROIAgent


def _save_run(sim_result: Dict[str, Any]) -> Path:
    """Guarda los resultados de una simulación en `data/bobina_simulator/completed_runs/`.
...
def main() -> None:
    st.set_page_config(page_title="BOBINA LOAD SIMULATOR", layout="wide")

    st.title("BOBINA LOAD SIMULATOR — Carretillas vs INGETRANS")
    ...
    with st.sidebar.expander("Configuración de Planta", expanded=True):
        ...
        num_portabobinas = st.number_input("Portabobinas (n)", value=cfg.get("num_spindles", 10))
        ...
        if st.button("Guardar configuración"):
            ...
            cfg["num_spindles"] = int(num_portabobinas)
            ...
            st.success("Configuración guardada en data/bobina_simulator/config_default.json")
